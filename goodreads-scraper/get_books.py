...
def get_format(soup):
    if soup.find('span', {'itemprop': 'bookFormat'}):
      book_format = soup.find('span', {'itemprop': 'bookFormat'}).text.strip()
      return book_format
    return ''

def get_language(soup):
    if soup.find('div', {'itemprop': 'inLanguage'}):
      language = soup.find('div', {'itemprop': 'inLanguage'}).text.strip()
      return language
    return ''

def get_description(soup):
  if soup.find('div', {'id': 'description'}):
    description = soup.find('div', {'id': 'description'}).text.strip()
    return description
  return ''

def get_coverimage(soup):
    if soup.find('div', {'class': 'editionCover'}):
        cover_image = soup.find('div', {'class': 'editionCover'}).img['src'].strip()
        return cover_image
    return ''
...
def scrape_book(book_id):
    ...
    return {'book_id_title':        book_id,
            ...
            'book_format':          get_format(soup),
            'language':             get_language(soup),
            'description':          get_description(soup),
            'cover_image':          get_coverimage(soup)}
...
def main():
    ...
    book_ids              = [line.strip() for line in open(args.book_ids_path, 'r', encoding='utf-8') if line.strip()]
